...
def get_popular_movies(user : str = Depends(get_current_user)):
    return getMPOAT()
...
def get_popular_genres(user : str = Depends(get_current_user)):
    return get_mpgh()

#Get most popular movies for this specific genres
@recommendation_router.get('/topGenreMovies/{genre_id}')
def get_top_genre_movies(genre_id : int, user : str = Depends(get_current_user)):
